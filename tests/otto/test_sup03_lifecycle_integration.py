...
def _integration(projects=None, host=None, binding=None, attention=None):
    ...
    integration = PortfolioLifecycleIntegration(
        ...
        projects=projects or {"project-a": "active", "project-b": "active"},
...
def test_real_request_path_closes_two_projects_to_zero_then_resumes_same_ids():
    ...
    first = integration.activate("project-a", request_id="activate-a", actor="manager-actor", generation=2)
    assert first["outcome"] == "reconciled"
    assert first["desired_states"] == {"manager": "active", "portfolio": "active"}

    manager_close = integration.manager_close("project-a", request_id="close-a", actor="manager-actor", generation=2)
    assert manager_close["outcome"] == "reconciled"
    assert manager_close["closing_project_ids"] == ["project-a"]

    terminal_a = integration.orchestrator_terminal_close("project-a", request_id="terminal-a", actor="orchestrator-actor", generation=2)
    assert terminal_a["outcome"] == "reconciled"
    assert terminal_a["desired_states"] == {"manager": "active", "portfolio": "active"}

    integration.manager_close("project-b", request_id="close-b", actor="manager-actor", generation=2)
    zero = integration.orchestrator_terminal_close("project-b", request_id="terminal-b", actor="orchestrator-actor", generation=2)
    ...
    resumed = integration.resume("project-c", request_id="activate-c", actor="orchestrator-actor", generation=2)
...
def test_manager_and_orchestrator_authority_and_blocked_terminal_boundary():
    ...
    with pytest.raises(LifecycleIntegrationError, match="foreign lifecycle authority"):
        integration.manager_close("project-a", request_id="foreign-close", actor="orchestrator-actor", generation=2)
    blocked = integration.manager_close("project-a", request_id="blocked-close", actor="manager-actor", generation=2, blocked=True)
    assert blocked["outcome"] == "reconciled"
    held = integration.orchestrator_terminal_close("project-a", request_id="blocked-terminal", actor="orchestrator-actor", generation=2)
...
def test_lost_response_replay_recovers_without_second_update():
    ...
    host.lost_reads = 2
    lost = integration.activate("project-a", request_id="lost-activate", actor="manager-actor", generation=2)
    ...
    replay = integration.activate("project-a", request_id="lost-activate", actor="manager-actor", generation=2)
    ...
    restarted_replay = restarted.resume("project-a", request_id="lost-activate", actor="manager-actor", generation=2)
...
def test_failed_pause_is_pending_then_reconciles_with_a_new_owner_request():
    integration, _attention, due, host = _integration({"project-a": "active"})
    assert integration.activate("project-a", request_id="initial", actor="manager-actor", generation=2)["outcome"] == "reconciled"
    assert integration.manager_close("project-a", request_id="close", actor="manager-actor", generation=2)["outcome"] == "reconciled"
    host.fail_pauses = True
    failed = integration.orchestrator_terminal_close("project-a", request_id="terminal-fail", actor="orchestrator-actor", generation=2)
...
def test_rebind_fences_old_generation_and_owner_while_new_owner_reconciles():
    ...
    with pytest.raises(LifecycleIntegrationError, match="stale lifecycle generation"):
        integration.activate("project-a", request_id="old-generation", actor="manager-actor", generation=2)
    fresh = integration.activate("project-a", request_id="new-generation", actor="manager-actor", generation=3)
